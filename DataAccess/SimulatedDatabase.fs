...
let mutable private stockData: StockData =
    ...
    { Bins = Set.ofList bins
      Content = Map.ofList [ bins.[1], products.[0]; bins.[3], products.[1]; bins.[4], products.[2] ]
      BinStructure =
        Map.ofList
...
type SimulatedDatabaseError = | BinAlreadyStored

/// Retrieves all bins from the simulated Storage Machine DB. These are all bins currently stored.
let retrieveBins () : Set<BinIdentifier> = stockData.Bins
...
let retrieveStock () : Map<BinIdentifier, PartNumber> = stockData.Content
...
let retrieveBinNesting () : Map<ParentBin, NestedBins> = stockData.BinStructure
